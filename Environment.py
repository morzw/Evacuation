...
class ReachedGoal(Instrument):
    ...
    def countReachedGoal(self, env):
        ...
        for agent in env.agents:
            if agent.pos.x > agent.goal.parameters['p1'].x or agent.pos.x <0:
                num_escaped += 1
...
def runSimulation(roomHeight=10,
    ...
    for _ in range(numAgents):
        ...
        pos = Point(roomWidth/2,roomHeight/2) # One agent
        # if rand<border:
        #     goal = goals[0]
        # else:
        # if pos.x > 7.5:
        ...
        #     goal = goals[1]
        goal = goals[0]
        agents.append(Agent(size, mass, pos, goal, desiredSpeed=Speed))
...
def show_graph(simResult):
    plt.style.use('dark_background')
    plt.plot(simResult, color = "red")
    #plt.plot(simResult, color="red")
    plt.xlabel("Num of steps")
    # plt.xlabel("Desired velocity")
    plt.ylabel("Num of escaped")
    # plt.ylabel("Time in seconds")
    plt.title('Simulation of max entities escaping a room with 2 doors in 90 seconds')
    plt.show()


if __name__ == '__main__':
    simResult = runSimulation(barrier=None, view=True, desiredSpeed=1.5, numAgents=1, roomHeight=20, roomWidth=20)
    print(simResult)

    show_graph(simResult)


    #thinkplot.plot(simResult)
    #thinkplot.show()
